...
def merge_original_data(directory, slice=["Latitude","Longitude"]):
    ...
    for filepath in Path(directory).glob('*.csv'):
        ...
        df = df.loc[:, slice]
        merged_df = pd.concat([merged_df, df])
    filtered_data = merged_df.loc[(merged_df['Latitude'] >= 50) & (merged_df['Latitude'] <= 66) & (merged_df['Longitude'] >= -10) & (merged_df['Longitude'] <= 30)]
    return merged_df

def convex_hull(df, filename = "convex_hull.kml"):
    sample = df.sample(frac = .01).to_numpy()
